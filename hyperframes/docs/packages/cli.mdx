...
<Tabs>
  <Tab title="Create">
    ...
    If an item declares `registryDependencies`, those items are resolved transitively and installed too, dependencies first. Each item is installed once even when several dependents share it, and the paste snippet always describes the item you asked for. Installed dependency names are listed in the command output and in the `dependencies` field of `--json`. A dependency cycle, or a dependency missing from the registry, fails the whole `add` before anything is written.

    Trying `add` with an example's name (e.g. `hyperframes add warm-grain`) emits a clear error pointing you at `init --example`. Examples are whole projects, so they are also rejected as dependencies of a block or component.
